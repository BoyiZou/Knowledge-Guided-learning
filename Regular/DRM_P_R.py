...
def trainnew(model,device,params,optimizer,scheduler):
    ...
    for step in range(params["trainStep"]):
        ...
        loss = torch.mean((0.5*dudx**2-2*u)) -params["penalty"]*torch.max(torch.abs((dudx[:,0])))
...
def test(model,device,params):
    ...
    dudx = torch.autograd.grad(u,data,grad_outputs=torch.ones_like(u),retain_graph=True,create_graph=True,only_inputs=True,allow_unused=True)[0]
    testloss = torch.mean((0.5*dudx**2-2*u))

    print("Test Error at Step is %s."%(testloss))
    testloss2 = torch.mean((0.5*dudx[5:,:]**2-2*u[5:,:]))
    ...
    plt.plot(data[:,0], 2*data[:,0]-data[:,0]**(2), color='k',linewidth=1)
    plt.legend(['Trained solution', 'Analytical solution'],markerscale=10,loc='upper left')
    plt.xlabel('x', fontsize=22)
    plt.ylabel('u', fontsize=22)
    plt.savefig('Penalty regular TT.eps',  format='eps')
    plt.savefig('Penalty regular TT.png',  format='png')
    plt.show()

    plt.figure()
    plt.scatter(data[:,0],abs(2*data[:,0].reshape(-1,1)-data[:,0].reshape(-1,1)**2-u),c='r', marker='x',s=2)
    plt.xlabel('x', fontsize=22)
    plt.ylabel('$\epsilon$', fontsize=22)
    plt.savefig('Penalty regular error.eps',  format='eps')
    plt.savefig('Penalty regular error.png',  format='png')
    plt.show()
...
#     model = RitzNet(params).to(device)
#     model.load_state_dict(torch.load('/content/last_model (1).pt'))
