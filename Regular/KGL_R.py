...
def trainnew(model,device,params,optimizer,scheduler):
    ...
    for step in range(params["trainStep"]):
        ...
        loss = torch.mean((0.5*dudx**2-2*u))

        if step < 30000:

          if step%5 >3:
            loss1 = -max(torch.abs((dudx[:,0])))
          else:
            loss1 = loss
        else:
          loss1 = loss
...
def test(model,device,params):
    ...
    plt.figure()
    plt.scatter(data[:,0], u, color='blue',s=0.45)
    ...
    plt.savefig('DRM regular TT.eps',  format='eps')
    plt.savefig('DRM regular TT.png',  format='png')
    plt.show()

    plt.figure()
    plt.scatter(data[:,0],abs(2*data[:,0].reshape(-1,1)-data[:,0].reshape(-1,1)**2-u),c='b',s=2)
    plt.xlabel('x', fontsize=22)
    plt.ylabel('$\epsilon$', fontsize=22)
    plt.savefig('DRM regular error.eps',  format='eps')
    plt.savefig('DRM regular error.png',  format='png')
    plt.show()
...
#     model = RitzNet(params).to(device)
#     model.load_state_dict(torch.load('/content/last_model_53.pt'))
