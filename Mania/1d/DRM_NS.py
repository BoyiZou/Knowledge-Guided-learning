...
def trainnew(model,device,params,optimizer,scheduler):
    ...
    data[0:params["bodyBatch_half"],0] = torch.from_numpy(np.random.rand(params["bodyBatch_half"]))
    data[params["bodyBatch_half"]:,0] = 0.25*torch.from_numpy(np.random.rand(params["bodyBatch_half"]))
...
def main():
    ...
    startTime = time.time()
    trainnew(model,device,params,optimizer,scheduler)  #trainnew model
