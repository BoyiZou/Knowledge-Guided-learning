...
class RitzNet(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.linearOut(x)
        y = x.clone()
        y[:,0] = (x[:,0].reshape(-1,1)*(xxx)*(1-xxx) + xxx).squeeze()
        x = y
...
def trainnew(model,device,params,optimizer,scheduler):
    ...
    for step in range(params["trainStep"]):
        ...
        loss = torch.mean((u**3-xx)**2*(v)**6) + params["penalty"]*torch.mean((dudx-v)**2)
...
def test(model,device,params):
    ...
    plt.ylabel('u_x', fontsize=22)
    plt.show()

    ppp = plt.figure()
    plt.scatter(data[:,0],v,c='y',s=0.05)
    plt.xlabel('x', fontsize=22)
